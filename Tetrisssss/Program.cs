...
namespace Tetris
{
    class Program
    {
        ...
        static char KarakterSementara = 'O';
 ...
 {{1,0},{1,1},{0,1},{1,2}}
        ...
        static void Main()
        {
            ...
            // input thread untuk dapet live input
            ...
            while (true)
            {
                ...
                if (timer >= maxTime)
                {
                    // kalo gak nabrak turun lek nabrak manggil BlockDownCollision
                    if (!Collision(IndexSekarang, bg, PosisiSementaraX, PosisiSementaraY + 1,
                   PosisiSementaraRotasi))
                    {
                        PosisiSementaraY++;
                    }
                    else
        ...
        static void tombolGerak()
        {
            switch (input.Key)
            {
                ...
                case ConsoleKey.LeftArrow:
                    if (!Collision(IndexSekarang, bg, PosisiSementaraX - 1, PosisiSementaraY,
                   PosisiSementaraRotasi))
                    {
                        PosisiSementaraX -= 1;
                    }
                ...
                case ConsoleKey.UpArrow:
                    ...
                    if (!Collision(IndexSekarang, bg, PosisiSementaraX, PosisiSementaraY, Rotasi))
                    {
                        PosisiSementaraRotasi = Rotasi;
                    }
                    break;
                // Hard drop = spasi
                case ConsoleKey.Spacebar:
                    ...
                    {
                        i++;
                        if (Collision(IndexSekarang, bg, PosisiSementaraX, PosisiSementaraY + i,
                       PosisiSementaraRotasi))
                        {
                ...
                case ConsoleKey.Enter:
                    ...
                    else
                    {
                        if (!Collision(holdIndex, bg, PosisiSementaraX, PosisiSementaraY, 0)) // cek tabrakan
                        {
                            ...
                            int c = IndexSekarang;
                            char ch = KarakterSementara;
                            IndexSekarang = holdIndex;
                            KarakterSementara = holdChar;
                            holdIndex = c;
                            holdChar = ch;
                        }
                    }
        ...
        {
            // Mengubah Block yang dimainkan menjadi background
            for (int i = 0; i < positions.GetLength(2); i++)
            {
                bg[positions[IndexSekarang, PosisiSementaraRotasi, i, 1] + PosisiSementaraY,
               positions[IndexSekarang, PosisiSementaraRotasi, i, 0] + PosisiSementaraX] = KarakterSementara;
            }
            // Loop 
            while (true)
            {
        ...
        static char[,] tampilanMapGameBerjalan()
        {
            ...
            for (int i = 0; i < positions.GetLength(2); i++)
            {
                view[positions[IndexSekarang, PosisiSementaraRotasi, i, 1] + PosisiSementaraY,
               positions[IndexSekarang, PosisiSementaraRotasi, i, 0] + PosisiSementaraX] = KarakterSementara;
            }
            return view;
        }
        static char[,] blokYangKeHold()
        {
        ...
        static char[,] nextBlockTampil()
        {
            ...
            for (int i = 0; i < 3; i++) // 3 Block selanjutnya
            {
                for (int l = 0; l < positions.GetLength(2); l++)
                {
                    ...
                    {
                        next[positions[nextBag[nextBagIndex], 0, l, 1] + 5 * i, positions[nextBag[nextBagIndex],
                       0, l, 0] + 1] = characters[nextBag[nextBagIndex]];
                    }
                    else
                    {
                        next[positions[bag[bagIndex + i], 0, l, 1] + 5 * i, positions[bag[bagIndex + i], 0, l, 0] + 1] =
                       characters[bag[bagIndex + i]];
                    }
                }
                if (i + bagIndex >= 7)
                {
        ...
        static void Print(char[,] view, char[,] hold, char[,] next)
        {
            for (int y = 0; y < baris; y++)
            {
                for (int x = 0; x < holdSizeX + kolom + upNextSize; x++)
                {
                    ...
                    else if (x >= holdSizeX + kolom)
                    {
                        i = next[y, x - kolom - upNextSize];
                    }
                    else
                    {
        ...
        static bool Collision(int index, char[,] bg, int x, int y, int rot)
        {
            ...
            for (int i = 0; i < positions.GetLength(2); i++)
            {
                ...
                if (positions[index, rot, i, 1] + y >= baris || positions[index, rot, i, 0] + x < 0 ||
               positions[index, rot, i, 0] + x >= kolom)
                {
                    return true;
                }
                // Check if not '-'
        ...
        static void BlockBaru()
        {
            ...
            PosisiSementaraY = 0;
            PosisiSementaraX = 4;
            KarakterSementara = characters[bag[bagIndex]];
            IndexSekarang = bag[bagIndex];
            // Jika Posisi Block Tabrakan Game over
            if (Collision(IndexSekarang, bg, PosisiSementaraX, PosisiSementaraY, PosisiSementaraRotasi)
           && amount > 0)
            {
                GameOver();
            }
